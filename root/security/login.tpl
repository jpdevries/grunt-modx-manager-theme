...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="{$_config.manager_url}templates/{%= prefix %}/css/login.css" />
...
<body id="login">
<div id="{%= cssprefix %}-login-language-select-div">
    <label>{$language_str}:
    <select name="cultureKey" id="{%= cssprefix %}-login-language-select">
        {$languages}
    </select>
    </label>
</div>
...
<div id="container">
    <div id="{%= cssprefix %}-login-logo">
        <img src="{$_config.manager_url}templates/{%= prefix %}/images/style/modx-logo-color.png" alt="" />
    </div>

<div id="{%= cssprefix %}-panel-login-div" class="x-panel modx-form x-form-label-right">
...
<form id="{%= cssprefix %}-login-form" action="" method="post">
    ...
    <div class="x-form-item login-form-item login-form-item-first">
      <div class="x-form-element login-form-element">
        <input type="text" id="{%= cssprefix %}-login-username" name="username" tabindex="1" autocomplete="on" value="{$_post.username}" class="x-form-text x-form-field" placeholder="{$_lang.login_username}" />
      </div>
    </div>

    <div class="x-form-item login-form-item">
      <div class="x-form-element login-form-element">
        <input type="password" id="{%= cssprefix %}-login-password" name="password" tabindex="2" autocomplete="on" class="x-form-text x-form-field" placeholder="{$_lang.login_password}" />
      </div>
    </div>
    ...
    <div class="login-cb-row">
        <div class="login-cb-col one">
            <div class="x-form-check-wrap modx-login-rm-cb">
                <input type="checkbox" id="{%= cssprefix %}-login-rememberme" name="rememberme" tabindex="3" autocomplete="on" {if $_post.rememberme}checked="checked"{/if} class="x-form-checkbox x-form-field" value="1" />
        ...
        <div class="login-cb-col two">
            <div class="modx-login-fl-link">
               <a href="javascript:void(0);" id="{%= cssprefix %}-fl-link" style="{if $_post.username_reset}display:none;{/if}">{$_lang.login_forget_your_login}</a>
            </div>
        </div>
    </div>
    ...
    <button class="x-btn-text login-form-btn" name="login" type="submit" value="1" id="{%= cssprefix %}-login-btn" tabindex="4">{$_lang.login_button}</button>
</form>

    {if $allow_forgot_password}
    <div class="modx-forgot-login">
    <form id="{%= cssprefix %}-fl-form" action="" method="post">
       <div id="{%= cssprefix %}-forgot-login-form" style="{if NOT $_post.username_reset}display: none;{/if}">

           <div class="x-form-item login-form-item">
              <div class="x-form-element login-form-element">
                <input type="text" id="{%= cssprefix %}-login-username-reset" name="username_reset" class="x-form-text x-form-field" value="{$_post.username_reset}" placeholder="{$_lang.login_username}" />
              </div>
           ...
           <button class="x-btn-text login-form-btn" name="forgotlogin" type="submit" value="1" id="{%= cssprefix %}-fl-btn">{$_lang.login_send_activation_email}</button>
